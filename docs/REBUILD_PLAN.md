...
| WP-07 | ✅ done | 2026-07-13 | FINAL WP — ALL 14 COMPLETE. Sonnet builder + architect (595 unit green; e2e green under CI retry policy). Separate per-image-priced AI_IMAGE_MODELS registry (gemini-3.1-flash-image default 4.5ct, gpt-image-2 5.3ct, gpt-image-1.5 — verified 2026-07-13), real callImage (Google generateContent + OpenAI images API, encrypted-key fallback chain, ledgered per image), imageGenService with in-service card-hero protection, fire-and-forget generation on plan save (never blocks), Nieuwe-foto button with cost confirm, hero toggle for card recipes, resumable/stoppable backfill, shimmer UI, settings model dropdown (in-app taste test). Accepted deviations: 60s image timeout (doc reconciled), Vandaag shimmer SSR-only, photoStatus filter on GET /api/recipes, photo e2e desktop-only with seed reset (TESTING §8 class). |
